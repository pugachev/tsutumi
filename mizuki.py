...
def upload_photo(file_path, creds, album_id, deleting_files=None):
    ...
    if not creds:
        print("✗ 認証トークンの取得に失敗しました:", file_path)
        return False, creds
...
class PhotoHandler(FileSystemEventHandler):
    ...
    def refresh_creds(self):
        """認証トークンをリフレッシュして更新"""
        refreshed_creds = refresh_credentials_if_needed(self.creds)
        if refreshed_creds:
            self.creds = refreshed_creds
            return True
        return False
    ...
    def on_created(self, event):
        if not event.is_directory and not event.src_path.endswith(".crdownload"):
            ...
            if wait_for_complete(file_path):
                ...
                # アップロード前にトークンをリフレッシュ（upload_photo内でもリフレッシュされるが、念のため）
                self.refresh_creds()
                # upload_photo内でcredsが更新される可能性があるため、戻り値で更新されたcredsを受け取る
                success, updated_creds = upload_photo(file_path, self.creds, self.album_id, self.deleting_files)
                # 更新されたcredsを保存
                if updated_creds:
                    self.creds = updated_creds
            else:
